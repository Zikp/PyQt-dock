...
from PyQt5.QtCore import (QByteArray, QSettings, QXmlStreamReader,
                          QXmlStreamWriter, pyqtSignal)
from PyQt5.QtGui import QIcon
from PyQt5.QtWidgets import QAction, QMainWindow, QMenu, QWidget

from qtpydocking.enums import InsertionOrder, DockFlags, DockWidgetArea, OverlayMode

from qtpydocking.dock_container_widget import DockContainerWidget
from qtpydocking.dock_overlay import DockOverlay
from qtpydocking.floating_dock_container import FloatingDockContainer
from qtpydocking.util import LINUX
...
class DockManagerPrivate:
    ...
    def restore_state_from_xml(self, state: QByteArray, version: int, testing: bool) -> bool:
        ...
        logger.debug(f'dock_containers {dock_containers}', )
    ...
    def restore_state(self, state: QByteArray, version: int) -> bool:
        ...
        if not self.check_format(state, version):
            logger.debug(f'checkFormat: Error checking format!')
            return False
        ...
        if not self.restore_state_from_xml(state, version, testing=False):
            logger.debug(f'restoreState: Error restoring state!')
            return False
    ...
    def restore_container(self, index: int, stream: QXmlStreamReader,
        ...
        if index >= len(self.containers):
            ...
            logger.debug(f'containers[{index}].restore_state()', )
...
class DockManager(DockContainerWidget):
    ...
    perspective_list_changed = pyqtSignal()

    # This signal is emitted if perspectives have been removed
    perspectives_removed = pyqtSignal()
    ...
    def register_floating_widget(self, floating_widget: FloatingDockContainer):
        ...
        logger.debug(f'floating widgets count = {len(self._mgr.floating_widgets)}',
                     )
    ...
    def dock_containers(self) -> list:
        ...
        for container in list(self._mgr.containers):
            try:
                ...
                logger.debug(f'qtpydocking TODO, container deleted',
                             exc_info=ex)
