...
from PyQt5.QtCore import (QByteArray, QEvent, QPoint, QXmlStreamReader,
                          QXmlStreamWriter, Qt, pyqtSignal)
from PyQt5.QtWidgets import QFrame, QGridLayout, QSplitter, QWidget

from qtpydocking.util import (find_parent, hide_empty_parent_splitters,
                              emit_top_level_event_for_widget, find_child, find_children)
from qtpydocking.enums import (DockWidgetArea, DockWidgetFeature, TitleBarButton,
...
if TYPE_CHECKING:
    from qtpydocking import DockManager, DockWidget, FloatingDockContainer
...
def replace_splitter_widget(splitter: QSplitter, from_: QWidget, to: QWidget):
    ...
    logger.debug(f'replace splitter widget {index} {from_} -> {to}')
...
class DockContainerWidgetPrivate:
    ...
    def dock_widget_into_dock_area(self, area: DockWidgetArea,
        ...
        if target_area_splitter.orientation() == insert_param.orientation:
            logger.debug(f'TargetAreaSplitter.orientation() == insert_orientation')
    ...
    def drop_into_section(self, floating_widget: 'FloatingDockContainer',
        ...
        logger.debug(f'Deleting floating_widget {floating_widget}', )
        floating_widget.deleteLater()
        self.add_dock_areas_to_list(new_dock_areas)
        self.public.dump_layout()
    ...
    def save_child_nodes_state(self, stream: QXmlStreamWriter, widget: QWidget):
        ...
        if isinstance(widget, QSplitter):
            ...
            logger.debug(f'NodeSplitter orient: {orientation} WidgetCount: {splitter.count()}')
    ...
    def restore_child_nodes(self, stream: QXmlStreamReader,
        ...
        while stream.readNextStartElement():
            ...
            logger.debug(f'restored child node {stream.name()}: {widget}')
    ...
    def restore_splitter(self, stream: QXmlStreamReader, testing: bool
        ...
        logger.debug(f'Restore NodeSplitter Orientation: {orientation}  WidgetCount: {widget_count}'
                     )
        ...
        while stream.readNextStartElement():
            ...
            if stream.name() == "Splitter":
                ...
                logger.debug(f'Sizes: {sizes} (from s_sizes: {s_sizes})')
            else:
                stream.skipCurrentElement()

            if splitter is not None and child_node is not None:
                logger.debug(
                    f'ChildNode isVisible {child_node.isVisible()} isVisibleTo {child_node.isVisibleTo(splitter)}')
    ...
    def restore_dock_area(self, stream: QXmlStreamReader, testing: bool) -> Tuple[bool, QWidget]:
        ...
        logger.debug(f'Restore NodeDockArea Tabs: {tabs} current: {current_dock_widget}')
        dock_area = None
        if not testing:
        ...
        while stream.readNextStartElement():
            ...
            if dock_widget and dock_area:
                logger.debug(f'Dock Widget found - parent {dock_widget.parent()}')
    ...
    def dump_recursive(self, level: int, widget: QWidget):
        ...
        if isinstance(widget, QSplitter):
            splitter = widget
            logger.debug(
                f"{indent}Splitter {('|' if splitter.orientation() == Qt.Vertical else '--')} v: {(' ' if splitter.isHidden() else 'v')} c: {splitter.count()}",
            )
            ...
            dock_area = widget
            logger.debug(f'{indent}DockArea', )
            logger.debug(
                f"{indent}{' ' if dock_area.isHidden() else 'v'} {' ' if dock_area.open_dock_widgets_count() > 0 else 'c'} DockArea"
            )

            indent = ' ' * (level + 1) * 4
            for i, dock_widget in enumerate(dock_area.dock_widgets()):
                logger.debug(
                    f"{indent}{'*' if i == dock_area.current_index() else ' '}{' ' if i == dock_widget.isHidden() else 'v'}{'c' if i == dock_widget.is_closed() else ' '} {dock_widget.windowTitle()}"
                )
...
class DockContainerWidget(QFrame):
    ...
    dock_areas_added = pyqtSignal()

    # This signal is emitted if one or multiple dock areas has been removed
    dock_areas_removed = pyqtSignal()

    # This signal is emitted if a dock area is opened or closed via
    # toggleView() function
    dock_area_view_toggled = pyqtSignal(DockAreaWidget, bool)
    ...
    def drop_floating_widget(self,
        ...
        target_pos : QPoint
        '''
        logger.debug(f'DockContainerWidget.dropFloatingWidget')
        ...
        if dock_area is not None:
            ...
            if drop_area != DockWidgetArea.invalid:
                logger.debug(f'Dock Area Drop Content: {drop_area}', )
        ...
        if DockWidgetArea.invalid == drop_area:
            ...
            logger.debug(f'Container Drop Content: {drop_area}', )
            if drop_area != DockWidgetArea.invalid:
    ...
    def remove_dock_area(self, area: DockAreaWidget):
        ...
        logger.debug(f'DockContainerWidget.removeDockArea')
        if area not in self.d.dock_areas:
            logger.error(f'Area {area} not found in DockContainerWidget {self}?')
            return
        ...
        logger.debug(f'area setParent {area} None')
        area.setParent(None)
        ...
        if splitter is self.d.root_splitter:
            logger.debug(f'Removed from RootSplitter')
    ...
    def save_state(self, stream: QXmlStreamWriter):
        ...
        stream : QXmlStreamWriter
        '''
        logger.debug(f'DockContainerWidget.saveState isFloating {self.is_floating()}',
                     )
        ...
        if self.is_floating():
            ...
            geo_str = str(geometry.toHex(' '), 'utf-8')
            stream.writeTextElement("Geometry", geo_str)
    ...
    def restore_state(self, stream: QXmlStreamReader, testing: bool = False) -> bool:
        ...
        logger.debug(f'Restore DockContainerWidget Floating {is_floating}', )
        ...
        if is_floating:
            logger.debug(f'Restore floating widget')
            ...
            geometry = QByteArray.fromHex(QByteArray(bytes(geometry_string, encoding='utf-8')))
            if geometry.isEmpty():
                return False
