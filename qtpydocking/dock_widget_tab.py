...
from PyQt5.QtCore import QEvent, QPoint, QSize, Qt, pyqtSignal, pyqtProperty
from PyQt5.QtGui import QContextMenuEvent, QCursor, QFontMetrics, QIcon, QMouseEvent
from PyQt5.QtWidgets import (QBoxLayout, QFrame, QLabel, QMenu, QSizePolicy,
                             QStyle, QWidget, QPushButton)

from qtpydocking.util import start_drag_distance, set_button_icon
from qtpydocking.enums import DragState, DockFlags, DockWidgetArea, DockWidgetFeature
from qtpydocking.eliding_label import ElidingLabel

if TYPE_CHECKING:
    from qtpydocking import (DockWidget, DockAreaWidget, FloatingDockContainer)
...
class DockWidgetTabPrivate:
    ...
    def start_floating(
        ...
        logger.debug(f'is_floating {dock_container.is_floating()}',
                     )
        logger.debug(f'area_count {dock_container.dock_area_count()}',
                     )
        logger.debug(f'widget_count {self.dock_widget.dock_area_widget().dock_widgets_count()}',
                     )
...
class DockWidgetTab(QFrame):
    active_tab_changed = pyqtSignal()
    clicked = pyqtSignal()
    close_requested = pyqtSignal()
    close_other_tabs_requested = pyqtSignal()
    ...
    def get_activate_tab(self):
        return self.is_active_tab()

    activeTab = pyqtProperty(bool, fget=get_activate_tab)
