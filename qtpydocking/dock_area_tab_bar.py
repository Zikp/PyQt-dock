...
from PyQt5.QtWidgets import QBoxLayout, QFrame, QScrollArea, QSizePolicy, QWidget

from qtpydocking.util import start_drag_distance, event_filter_decorator
from qtpydocking.enums import DragState, DockWidgetArea
...
if TYPE_CHECKING:
    from qtpydocking import DockAreaWidget
...
class DockAreaTabBar(QScrollArea):
    ...
    # removed
    removing_tab = pyqtSignal(int)

    # This signal is emitted if a tab has been inserted
    tab_inserted = pyqtSignal(int)
    ...
    def on_tab_widget_moved(self, global_pos: QPoint):
        ...
        if to_index >= 0:
            logger.debug(f'tabMoved from {from_index} to {to_index}')
    ...
    def mouseReleaseEvent(self, ev: QMouseEvent):
        ...
        logger.debug(f'DockAreaTabBar.mouseReleaseEvent')
        ev.accept()
    ...
    def mouseMoveEvent(self, ev: QMouseEvent):
        ...
        if drag_distance >= start_drag_distance():
            logger.debug(f'DockAreaTabBar.startFloating')
    ...
    def remove_tab(self, tab: 'DockWidgetTab'):
        ...
        logger.debug(f'DockAreaTabBar.removeTab')
        ...
        logger.debug(f'NewCurrentIndex {new_current_index}')
