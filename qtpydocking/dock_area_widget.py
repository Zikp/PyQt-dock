...
from PyQt5.QtCore import QRect, QXmlStreamWriter, pyqtSignal
from PyQt5.QtWidgets import QAbstractButton, QAction, QBoxLayout, QFrame

from qtpydocking.util import (find_parent, DEBUG_LEVEL, hide_empty_parent_splitters,
                              emit_top_level_event_for_widget)
from qtpydocking.enums import TitleBarButton, DockWidgetFeature
from qtpydocking.dock_area_layout import DockAreaLayout

if TYPE_CHECKING:
    from qtpydocking import (DockContainerWidget, DockManager, DockWidget, DockWidgetTab,
...
class DockAreaWidget(QFrame):
    ...
    tab_bar_clicked = pyqtSignal(int)
    ...
    current_changing = pyqtSignal(int)
    ...
    def on_tab_close_requested(self, index: int):
        ...
        index : int
        '''
        logger.debug(f'DockAreaWidget.onTabCloseRequested %s', index)
    ...
    def reorder_dock_widget(self, from_index: int, to_index: int):
        ...
        to_index : int
        '''
        logger.debug(f'DockAreaWidget.reorderDockWidget')
        if (from_index >= self.d.contents_layout.count() or
                from_index < 0 or
                to_index >= self.d.contents_layout.count() or
                to_index < 0 or
                from_index == to_index):
            logger.debug(f'Invalid index for tab movement %s:%s', from_index,
                         to_index)
            return
    ...
    def remove_dock_widget(self, dock_widget: 'DockWidget'):
        ...
        dock_widget : DockWidget
        '''
        logger.debug(f'DockAreaWidget.removeDockWidget')
        ...
        if next_open_dock_widget is not None:
            ...
            logger.debug(f'Dock Area empty')
    ...
    def save_state(self, stream: QXmlStreamWriter):
        ...
        logger.debug(f'DockAreaWidget.saveState TabCount: {self.d.contents_layout.count()} current: {name}'
                     , )
