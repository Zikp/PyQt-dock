...
from PyQt5.QtCore import (QEvent, QObject, QPoint, QRect, QSize, QXmlStreamReader, Qt)
from PyQt5.QtGui import QCloseEvent, QCursor, QGuiApplication, QHideEvent, QMoveEvent
from PyQt5.QtWidgets import QApplication, QBoxLayout, QWidget, QDockWidget

from qtpydocking.enums import DockWidgetFeature, DragState, DockWidgetArea
from qtpydocking.util import LINUX, event_filter_decorator, QT_VERSION_TUPLE
from qtpydocking.dock_container_widget import DockContainerWidget
from qtpydocking.floating_widget_title_bar import FloatingWidgetTitleBar

if TYPE_CHECKING:
    from qtpydocking import DockAreaWidget, DockWidget, DockManager
...
class FloatingDockContainerPrivate:
    ...
    def title_mouse_release_event(self):
        ...
        if not self.drop_container:
            logger.debug(f'title_mouse_release_event: no drop container?')
            return
        ...
        if any(widget.drop_area_under_cursor() != DockWidgetArea.invalid
            ...
            if not rect.isValid():
                logger.debug(
                    f'title_mouse_release_event: invalid rect ' + f'x {rect.x()} y {rect.y()} w {rect.width()} h {rect.height()}'
                    )
            else:
                public = self.public
    ...
    def update_drop_overlays(self, global_pos: QPoint):
        ...
        if not top_container:
            logger.debug(f'update_drop_overlays: No top container')
            container_overlay.hide_overlay()
            dock_area_overlay.hide_overlay()
            return

        logger.debug(
            f'update_drop_overlays: top container={self.drop_container} name={self.drop_container.objectName()}'
            )
...
class FloatingDockContainer(FloatingWidgetBase):
    ...
    def on_dock_areas_added_or_removed(self):
        logger.debug(f'FloatingDockContainer.onDockAreasAddedOrRemoved()')
    ...
    def finish_dragging(self):
        ...
        finished
        '''
        logger.debug(f'FloatingDockContainer.finish_dragging')
        if LINUX:
            self.setAttribute(Qt.WA_X11NetWmWindowTypeDock, False)
            self.setWindowOpacity(1)
            self.activateWindow()
            if self.d.mouse_event_handler is not None:
                logger.debug(f'Mouse event handler releaseMouse')
    ...
    def changeEvent(self, event: QEvent):
        ...
        if (event.type() == QEvent.ActivationChange) and self.isActiveWindow():
            logger.debug(f'FloatingWidget.changeEvent QEvent.ActivationChange ')
            global _z_order_counter  # TODO
            _z_order_counter += 1
        ...
        if state == DragState.inactive:
            ...
            if e.type() == QEvent.NonClientAreaMouseButtonPress:
                if QT_VERSION_TUPLE >= (5, 12, 2):
                    ...
                    logger.debug(f'FloatingWidget.event Event.NonClientAreaMouseButtonPress {e.type()}', )
            ...
            if e.type() == QEvent.NonClientAreaMouseButtonDblClick:
                logger.debug(f'FloatingWidget.event QEvent.NonClientAreaMouseButtonDblClick')
            ...
            if e.type() == QEvent.NonClientAreaMouseButtonRelease:
                logger.debug(f'FloatingWidget.event QEvent.NonClientAreaMouseButtonRelease')
    ...
    def closeEvent(self, event: QCloseEvent):
        ...
        event : QCloseEvent
        '''
        logger.debug(f'FloatingDockContainer closeEvent')
        self.d.set_state(DragState.inactive)
    ...
    def eventFilter(self, watched: QObject, event: QEvent) -> bool:
        ...
        if event.type() == QEvent.MouseButtonRelease:
            logger.debug(f'MouseButtonRelease')
            if self.d.dragging_state == DragState.floating_widget:
                qapp = QApplication.instance()
                qapp.removeEventFilter(self)
                logger.debug(f'FloatingWidget.eventFilter QEvent.MouseButtonRelease')
